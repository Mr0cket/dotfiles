...
function create_droplet() {
	...
	while [[ $attempt -le $max_attempts ]]; do
		...
		ssh $ssh_options root@"$droplet_ip" <<EOF
            ...
            # sudo echo 'DPkg::Lock::Timeout 60' > /etc/apt/apt.conf.d/lock-timeout # Possible workaround to 'unable to acquire lock' issues
...
EOF
	...
	mkdir -p $OVPN_DIR
	scp root@"$droplet_ip":/root/$profile_name.ovpn $OVPN_DIR

	if [[ $? -ne 0 ]]; then
		echo "Error: Failed to download .ovpn file for user $profile_name."
		exit 1
	fi
	echo "Downloaded .ovpn file for user $profile_name: $OVPN_DIR/$profile_name.ovpn"
}
